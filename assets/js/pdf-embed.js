...
async function renderEmbed(root) {
  ...
  const padding = parseInt(root.getAttribute("data-padding") || "16", 10);
  ...
  try {
    ...
    // Render all pages (still fine for 1-pager)
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      const availableWidth = Math.min(pagesContainer.clientWidth, maxWidth) - 0;
      ...
      // Keep annotation layer aligned on responsive width changes
      const resize = () => {
    ...
    setStatus(
      root,
      `Could not display PDF. <a href="${pdfUrl}">Open the PDF</a>.`
    );
  }
}
